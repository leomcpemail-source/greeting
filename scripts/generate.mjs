...
async function main() {
  ...
  if (USE_POLL_VISION) voters.unshift({ name: 'pollinations', fn: visionPollRubric }); // เพิ่มเมื่อ Pollen/Pollinations พร้อม
  ...
  while (st.images.length < TARGET && timeLeft() > 90000 && gens < MAX_GEN_PER_RUN) {
    ...
    if (r.decision === 'keep') {
      ...
      // รูปถ่าย royalty-free + AI ล่มหมด → ลอง Gemini ตรวจก่อน 1 รอบ (กัน Pexels ส่งรูปผิดหมวด เช่น จระเข้/คน)
      let _photoOk = true;
      try {
        const _gv = await visionGemini(card, VISION_PROMPT);
        if (_gv && _gv.ok === false) {
          _photoOk = false;
          console.log(`  ✗ photo Gemini BAD (score ${_gv.score}) — ทิ้ง ${fname}`);
          try { fs.unlinkSync(path.join(dir, fname)); } catch{}
        } else {
          console.log(`  ✓ keep(photo, Gemini OK score ${_gv ? _gv.score : '?'}) ${fname}`);
        }
      } catch(_e) {
        console.log(`  ✓ keep(photo, AI ล่ม) ${fname}`);
      }
      if (!_photoOk) continue;
