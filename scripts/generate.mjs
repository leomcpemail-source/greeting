...
const VISION_PROMPT_COMPOSITE =
  ...
  + '5 = average; 1 = ugly or unreadable). Judge the frame, the text and the photo TOGETHER as one design.\n'
  ...
  while (images.length < TARGET && attempt < MAX_ATTEMPTS) {
    ...
    try {
      ...
      if (!overBudget) {
        ...
        // 2) คะแนนต่ำกว่าเกณฑ์ → ทิ้ง (ผ่อนเกณฑ์เมื่อใช้ attempts ไปมากแล้ว)
