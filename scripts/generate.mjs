...
async function genBlessingsGemini({ dayTh, headline, isFestival, n }) {
  ...
  // ลอง Gemini ก่อน (retry 1 ครั้งถ้า 429), fallback ไป Pollinations ถ้าไม่มี key
  if (GEMINI_API_KEY) {
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        if (attempt > 0) {
          console.log('Gemini retry หลัง delay 6 วิ...');
        ...
        console.log(`Gemini error: ${e.message}${attempt < 1 && e.message.includes('429') ? ' — จะ retry' : ' — fallback Pollinations'}`);
        if (!e.message.includes('429')) break; // ถ้าไม่ใช่ 429 ไม่ต้อง retry
      }
    }
  }

  // Fallback: Pollinations (เพิ่ม timeout เป็น 90 วิสำหรับ text gen)
  const TEXT_FALLBACK_TIMEOUT = 90000;
  try {
