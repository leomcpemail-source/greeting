...
async function main() {
  ...
  const tgt = new Date(ict.getTime() + 24 * 3600 * 1000);
  ...
  const keep = [...new Set([liveISO, targetISO])];
  ...
  ], { passScore: PASS_SCORE, minClarity: MIN_CLARITY, minVoters: 2 });
  ...
  while (st.images.length < TARGET && timeLeft() > 90000 && gens < MAX_GEN_PER_RUN) {
    ...
    try {
      if (useBank) {
      ...
      } else if (photosEnabled() && !fest && Math.random() < 0.35) {
        ...
        } catch (e) { console.log('  (photo fail, gen แทน):', e.message); }
      }
      if (!raw) {
    ...
    if (r.decision === 'keep') {
      ...
      console.log(`  … pending ${fname} (${r.reason})`);
    } else {
      fs.rmSync(path.join(dir, fname), { force: true });
      console.log(`  ✗ reject (${r.reason})`);
    }
    saveManifest(dir, st);
  }
