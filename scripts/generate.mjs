...
const MIN_SCORE = 5;              // คะแนนความสวยขั้นต่ำ (1-10) ต่ำกว่านี้ทิ้ง regen
const SCORE_RELAX_AT = 0.7;       // ถ้าใช้ attempts เกิน 70% แล้ว → ผ่อนเกณฑ์คะแนนลง (กันรูปไม่ครบ)
...
async function visionPollinations(buf) {
...
async function visionGemini(buf) {
...
async function assessImage(buf) {
  if (!VISION) return { ok: true, score: 7, textSafe: true, src: 'off' };

  // ── ด่าน 1: Pollinations ──
  const pol = await visionPollinations(buf);
  ...
  const gem = await visionGemini(buf);
...
async function genBlessingsGemini({ dayTh, headline, isFestival, n }) {
  ...
  + `- ความยาวแต่ละประโยค 10-25 คำ (ไม่สั้นหรือยาวเกินไป)\n`
  ...
  function parseArr(text) {
    ...
    if (!m) return [];
    try {
      return JSON.parse(m[0]).filter(x => typeof x === 'string' && x.trim().length > 8).map(x => x.trim());
    } catch { return []; }
  }
...
async function main() {
  ...
  const blessings = await genBlessingsGemini({
    dayTh: dayTheme.th,
    headline,
    isFestival: !!fest,
    n: TARGET
  ...
  while (images.length < TARGET && attempt < MAX_ATTEMPTS) {
    ...
    try {
      ...
      if (!overBudget) {
        const a = await assessImage(buf);
        score = a.score; textSafe = a.textSafe; src = a.src;
        // 1) รูปพัง/ไม่เหมาะสม → ทิ้ง
        if (!a.ok) { console.log(`  ✗ vision: BAD ทิ้ง (${src})`); continue; }
        // 2) คะแนนความสวยต่ำกว่าเกณฑ์ → ทิ้ง (ผ่อนเกณฑ์เมื่อใช้ attempts ไปมากแล้ว กันรูปไม่ครบ)
        const relaxed = attempt > MAX_ATTEMPTS * SCORE_RELAX_AT;
        ...
        // 3) (ออปชัน) ตรงกลางรกเกินไปจนวางข้อความไม่สวย → ทิ้ง
        if (REQUIRE_TEXT_SAFE && !textSafe && !relaxed) { console.log('  ✗ ตรงกลางรกเกินไป (วางข้อความไม่สวย) ทิ้ง'); continue; }
      }

      const idx = String(images.length).padStart(2,'0');
      fs.writeFileSync(path.join(IMG_DIR, `${idx}.jpg`), buf);
      images.push(`img/${idx}.jpg`);
      scored.push({ file: `img/${idx}.jpg`, score, textSafe });
      console.log(`  ✓ เก็บเป็น ${idx}.jpg (คะแนน ${score}·${src}${textSafe?'':' /ตรงกลางรก'})`);
    } catch (e) {
  ...
  console.log(`=== เสร็จ: ${images.length} รูป (คะแนนเฉลี่ย ${avgScore ?? '-'}), ${blessings.length} คำอวยพร${fest?' (เทศกาล: '+fest.headline+')':''} (ใช้เวลา ${Math.round((Date.now()-T0)/60000)} นาที) ===`);
}
