...
const GEMINI_VISION_MODELS = ['gemini-3-flash-preview', 'gemini-2.5-flash', 'gemini-3.5-flash']; // เรียงตัวที่ "ใช้ได้จริง" ขึ้นก่อน — gemini-3.5-flash มัก timeout/aborted จึงย้ายไปท้าย (free tier ทั้งหมด)
const GROQ_VISION = true;   // ชั้น 3 (เร็วสุด): ใช้ Groq (Llama-4 Scout vision) เป็น fallback แรกเมื่อ Pollinations/Gemini ล่ม — Groq เร็วกว่า OpenRouter
const GROQ_VISION_MODELS = ['meta-llama/llama-4-scout-17b-16e-instruct']; // โมเดล vision ฟรีบน Groq (OpenAI-compatible) — ใช้ GROQ_API_KEY ตัวเดียวกับ gen คำอวยพร
const OPENROUTER_VISION = true;   // ชั้น 3: ใช้ OpenRouter (vision) เป็น fallback เมื่อ Pollinations+Gemini ล่มทั้งคู่ — ปิดได้ถ้าไม่อยากใช้โควตา
const OPENROUTER_VISION_MODELS = ['meta-llama/llama-4-maverick:free', 'google/gemma-3-27b-it:free']; // โมเดลฟรีที่ "ดูรูปได้" บน OpenRouter (ลองตามลำดับ) — ใช้ OPENROUTER_API_KEY ตัวเดียวกับ gen คำอวยพร
...
// ด่าน 3: OpenRouter (vision) — fallback สุดท้ายก่อนปล่อยผ่าน คืน {ok,score,textSafe} หรือ null
// ใช้ OPENROUTER_API_KEY ตัวเดียวกับ gen คำอวยพร ; OpenAI-compatible (ส่ง text ก่อน แล้วค่อย image ตามคำแนะนำ OpenRouter)
// ชั้น 3 (เร็วสุด): Groq vision (Llama-4 Scout) — OpenAI-compatible, ใช้ GROQ_API_KEY ตัวเดียวกับ gen คำอวยพร
async function visionGroq(buf, prompt = VISION_PROMPT) {
  if (!GROQ_VISION || !GROQ_API_KEY) return null;
  const b64 = buf.toString('base64');
    ...
    try {
      const res = await fetchT('https://api.groq.com/openai/v1/chat/completions',
        { method:'POST',
          ...
          body: JSON.stringify({
            model, max_completion_tokens: 24, temperature: 0.2,
            messages: [{ role:'user', content: [
              { type:'text', text: prompt },
              { type:'image_url', image_url: { url: `data:image/jpeg;base64,${b64}` } }
            ]}]
        ...
        TXT_TIMEOUT_MS);
      if (res.status === 429) { console.log(`    Groq ${model}: 429 (เต็มโควตา) — ลองตัวถัดไป`); continue; }
      if (!res.ok) { console.log(`    Groq ${model}: http ${res.status}`); continue; }
      const data = await res.json();
      const raw = (data?.choices?.[0]?.message?.content || '').trim();
      if (raw) return parseAssess(raw);
...
async function visionOpenRouter(buf, prompt = VISION_PROMPT) {
  if (!OPENROUTER_VISION || !OPENROUTER_API_KEY) return null;
  const b64 = buf.toString('base64');
    ...
    try {
      const res = await fetchT('https://openrouter.ai/api/v1/chat/completions',
        { method:'POST',
          ...
          body: JSON.stringify({
            model, max_tokens: 24, temperature: 0.2,
            messages: [{ role:'user', content: [
              { type:'text', text: prompt },
              { type:'image_url', image_url: { url: `data:image/jpeg;base64,${b64}` } }
            ]}]
        ...
        TXT_TIMEOUT_MS);
      if (res.status === 429) { console.log(`    OpenRouter ${model}: 429 (เต็มโควตา) — ลองตัวถัดไป`); continue; }
      if (!res.ok) { console.log(`    OpenRouter ${model}: http ${res.status}`); continue; }
      const data = await res.json();
      const raw = (data?.choices?.[0]?.message?.content || '').trim();
      if (raw) return parseAssess(raw);
    } catch (e) { console.log(`    OpenRouter ${model} error: ${e.message}`); }
  }
  return null;
}
...
async function assessImage(buf, composite = false) {
  ...
  if (!pol) {
    // Pollinations vision ล่ม → อย่าเพิ่งปล่อยผ่าน ลองให้ Gemini ตรวจแทนก่อน (Gemini ไม่ผ่าน gate ของ Pollinations จึงยังเรียกได้)
    if (GEMINI_VISION) {
      const gem = await visionGemini(buf, prompt);
      if (gem) return { ...gem, src: 'gemini(pol-fail)' };   // Gemini ตรวจให้แทน = กรองจริง
    }
    // ชั้น 3 (เร็วสุด): Groq → ชั้น 4: OpenRouter — เรียกเมื่อ Pollinations + Gemini ล่มทั้งคู่ ก่อนยอมปล่อยผ่าน
    const grq = await visionGroq(buf, prompt);
    if (grq) return { ...grq, src: 'groq(pol+gem-fail)' };          // Groq ตรวจให้ = กรองจริง
    const orr = await visionOpenRouter(buf, prompt);
    if (orr) return { ...orr, src: 'openrouter(pol+gem-fail)' };    // OpenRouter ตรวจให้ = กรองจริง
    return { ok: true, score: 6, textSafe: true, src: 'nofilter' }; // 4 ตัวล่มหมด → ปล่อยผ่าน (กันรอบพัง/ได้ 0 รูป) — grep 'nofilter' เพื่อดูรูปที่ไม่ได้ตรวจจริง
  }
  ...
  const gem = await visionGemini(buf, prompt);
  if (gem) return { ...gem, src: 'gemini' };
  // Gemini ล่มในเคสก้ำกึ่ง → ขอความเห็นที่สองจาก Groq → OpenRouter ก่อนถอยมาใช้ผล Pollinations
  const grq = await visionGroq(buf, prompt);
  if (grq) return { ...grq, src: 'groq(gem-fail)' };
  const orr = await visionOpenRouter(buf, prompt);
  if (orr) return { ...orr, src: 'openrouter(gem-fail)' };
  return { ...pol, src: 'pol(gem-fail)' };  // ทุกตัวสำรองล่ม → ใช้ผล Pollinations ที่ตรวจไว้แล้ว
}
...
async function genBlessingsGemini({ dayTh, headline, isFestival, n }) {
  ...
  // ── 1. Gemini 2.0 Flash ──
  if (GEMINI_API_KEY) {
    for (const model of ['gemini-3-flash-preview', 'gemini-2.5-flash', 'gemini-3.5-flash']) {
...
async function main() {
  ...
  while (images.length < TARGET && attempt < MAX_ATTEMPTS) {
    ...
    const overBudget = false;   // เลิกใช้โหมด "เกินงบ-ไม่กรอง" — ไม่ปล่อยรูปที่ยังไม่ผ่าน vision หลุดเด็ดขาด
    // เกินงบเวลา gen → หยุดทันที (เก็บเฉพาะรูปที่กรองผ่านแล้ว ดีกว่าได้รูปครบแต่มีรูปไม่กรองปน)
    if (elapsed > TIME_BUDGET_MS) {
      console.log(`[หยุด] เกินงบเวลา ${Math.round(elapsed/60000)} นาที — หยุด gen (เก็บเฉพาะรูปที่กรองผ่านแล้ว ${images.length} รูป)`);
      break;
    }
    // กันเหนียว: เลยเพดานเวลาสุดท้าย
    ...
    try {
      ...
      // วิเคราะห์ตำแหน่งข้อความที่ไม่บังจุดสำคัญ (บน/ล่าง) จากตัวภาพจริง
      const band = await bestTextBand(buf);   // 'top' | 'bottom' | null
      ...
      if (renderer && rCounts) {
        ...
        if (band) combo.vp = band;   // ตรึงตำแหน่งข้อความตามพื้นที่ว่างของรูป
      }
        ...
        if (renderer && combo) {
          try {
            const out = await renderer.render({
              ...
              dayTh: dayTheme.th, size: 800, vp: combo.vp || null
            });
      ...
      images.push(file);
      scored.push({ file, score, textSafe, band });
  ...
  const imgPos = scored.map(s => s.band || null);   // ตำแหน่งข้อความแนะนำต่อรูป (บน/ล่าง) เรียงตรงกับ images
  ...
  const manifest = {
    ...
    imgPos: imgPos.some(p => p) ? imgPos : undefined,  // 'top'/'bottom' ต่อรูป (วางข้อความไม่บังจุดสำคัญ) — ใช้ตอน composite ปิด
