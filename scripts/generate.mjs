...
const GEMINI_VISION_MODELS = ['gemini-3.5-flash', 'gemini-3-flash-preview', 'gemini-2.5-flash']; // ลองตามลำดับ (สูง→ต่ำ ทั้งหมด free tier)
...
async function genBlessingsGemini({ dayTh, headline, isFestival, n }) {
  ...
  // ── 1. Gemini 2.0 Flash ──
  if (GEMINI_API_KEY) {
    for (const model of ['gemini-3.5-flash', 'gemini-3-flash-preview', 'gemini-2.5-flash']) {
