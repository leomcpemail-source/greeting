...
const DAILY_BLESSINGS = 60;     // gen คำอวยพรให้เท่ากับจำนวนรูป (1 รูป 1 คำ)
...
async function genBlessingsGemini({ dayTh, headline, isFestival, n }) {
  ...
  // ลอง Gemini ก่อน, fallback ไป Pollinations ถ้าไม่มี key
  if (GEMINI_API_KEY) {
    try {
      console.log(`gen คำอวยพร ${n} คำ ด้วย Gemini Flash...`);
      ...
      if (!m) throw new Error('Gemini: ไม่พบ JSON array');
      const arr = JSON.parse(m[0]).filter(x => typeof x === 'string' && x.trim().length > 8).map(x => x.trim());
      if (arr.length < Math.floor(n * 0.5)) throw new Error(`Gemini: ได้คำน้อยเกินไป (${arr.length})`);
      console.log(`✓ Gemini ได้ ${arr.length} คำ`);
      return arr;
    } catch (e) {
      console.log(`Gemini error: ${e.message} — fallback Pollinations`);
    }
  }

  // Fallback: Pollinations
  try {
    console.log(`gen คำอวยพร ${n} คำ ด้วย Pollinations (fallback)...`);
    await gate();
    const res = await fetchT('https://text.pollinations.ai/openai',
      { method:'POST', headers:{'Content-Type':'application/json'},
  ...
  } catch (e) { console.log('genBlessings fallback error', e.message); return []; }
}
...
async function main() {
  ...
  // คำอวยพร: gen สดทุกวันด้วย Gemini (วันเทศกาลใช้ static 3 คำ + gen เพิ่ม)
  let blessings;
  if (fest && fest.blessings && fest.blessings.length) {
    const festStatic = fest.blessings;
    const need = Math.max(0, images.length - festStatic.length);
    const extra = need > 0
      ? await genBlessingsGemini({ dayTh: dayTheme.th, headline, isFestival: true, n: need })
      : [];
    blessings = [...festStatic, ...extra];
  } else {
    blessings = await genBlessingsGemini({ dayTh: dayTheme.th, headline, isFestival: false, n: images.length });
  }
